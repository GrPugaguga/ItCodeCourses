...
function formatDuration(seconds) {
    ...
    function strFormat(s, k) {
        if (timeStr == '') {
            timeStr += `${resultTime} ${k}${s}`
        } else if (timeStr != '' && seconds % timeObj[k] == 0) {
            timeStr += ` and ${resultTime} ${k}${s}`
        } else {
            timeStr += `, ${resultTime} ${k}${s}`
        }
    }
    ...
    for (let key of Object.keys(timeObj)) {
        resultTime = Math.floor(seconds / timeObj[key])
        seconds = seconds - resultTime * timeObj[key];
        if (resultTime > 1) {
            strFormat('s', key)
